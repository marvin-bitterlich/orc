...
func TestReceiptRepository_GetWOStatus(t *testing.T) {
	db := setupTestDB(t)
	repo := sqlite.NewReceiptRepository(db)
	ctx := context.Background()

	// Setup with Work Order
	db.ExecContext(ctx, "INSERT OR IGNORE INTO commissions (id, title, status) VALUES (?, ?, ?)", "COMM-001", "Test", "active")
	db.ExecContext(ctx, "INSERT INTO shipments (id, commission_id, title, status) VALUES (?, ?, ?, ?)", "SHIP-001", "COMM-001", "Test", "active")
	db.ExecContext(ctx, "INSERT INTO work_orders (id, shipment_id, outcome, status) VALUES (?, ?, ?, ?)", "WO-001", "SHIP-001", "Test outcome", "complete")

	t.Run("returns status for existing WO", func(t *testing.T) {
		status, err := repo.GetWOStatus(ctx, "SHIP-001")
		if err != nil {
			t.Fatalf("GetWOStatus failed: %v", err)
		}
		if status != "complete" {
	...
	t.Run("returns error for shipment without WO", func(t *testing.T) {
		_, err := repo.GetWOStatus(ctx, "SHIP-999")
		if err == nil {
			t.Error("expected error, got nil")
		}
	})
...
func TestReceiptRepository_AllCRECsVerified(t *testing.T) {
	db := setupTestDB(t)
	repo := sqlite.NewReceiptRepository(db)
	ctx := context.Background()

	// Setup: commission -> shipment -> cycle -> CWO
	db.ExecContext(ctx, "INSERT OR IGNORE INTO commissions (id, title, status) VALUES (?, ?, ?)", "COMM-001", "Test", "active")
	db.ExecContext(ctx, "INSERT INTO shipments (id, commission_id, title, status) VALUES (?, ?, ?, ?)", "SHIP-001", "COMM-001", "Test", "active")
	db.ExecContext(ctx, "INSERT INTO shipments (id, commission_id, title, status) VALUES (?, ?, ?, ?)", "SHIP-002", "COMM-001", "Test 2", "active")
	db.ExecContext(ctx, "INSERT INTO shipments (id, commission_id, title, status) VALUES (?, ?, ?, ?)", "SHIP-003", "COMM-001", "Test 3", "active")

	// SHIP-001: no cycles/CRECs (vacuously true)
	// SHIP-002: has cycles with all verified CRECs
	db.ExecContext(ctx, "INSERT INTO cycles (id, shipment_id, sequence_number, status) VALUES (?, ?, ?, ?)", "CYC-001", "SHIP-002", 1, "complete")
	db.ExecContext(ctx, "INSERT INTO cycles (id, shipment_id, sequence_number, status) VALUES (?, ?, ?, ?)", "CYC-002", "SHIP-002", 2, "complete")
	db.ExecContext(ctx, "INSERT INTO cycle_work_orders (id, cycle_id, shipment_id, outcome, status) VALUES (?, ?, ?, ?, ?)", "CWO-001", "CYC-001", "SHIP-002", "Test 1", "complete")
	...
	db.ExecContext(ctx, "INSERT INTO cycle_receipts (id, cwo_id, shipment_id, delivered_outcome, status) VALUES (?, ?, ?, ?, ?)", "CREC-001", "CWO-001", "SHIP-002", "Delivered 1", "verified")
	db.ExecContext(ctx, "INSERT INTO cycle_receipts (id, cwo_id, shipment_id, delivered_outcome, status) VALUES (?, ?, ?, ?, ?)", "CREC-002", "CWO-002", "SHIP-002", "Delivered 2", "verified")

	// SHIP-003: has cycles with some non-verified CRECs
	db.ExecContext(ctx, "INSERT INTO cycles (id, shipment_id, sequence_number, status) VALUES (?, ?, ?, ?)", "CYC-003", "SHIP-003", 1, "complete")
	db.ExecContext(ctx, "INSERT INTO cycles (id, shipment_id, sequence_number, status) VALUES (?, ?, ?, ?)", "CYC-004", "SHIP-003", 2, "active")
	db.ExecContext(ctx, "INSERT INTO cycle_work_orders (id, cycle_id, shipment_id, outcome, status) VALUES (?, ?, ?, ?, ?)", "CWO-003", "CYC-003", "SHIP-003", "Test 3", "complete")
	db.ExecContext(ctx, "INSERT INTO cycle_work_orders (id, cycle_id, shipment_id, outcome, status) VALUES (?, ?, ?, ?, ?)", "CWO-004", "CYC-004", "SHIP-003", "Test 4", "active")
	db.ExecContext(ctx, "INSERT INTO cycle_receipts (id, cwo_id, shipment_id, delivered_outcome, status) VALUES (?, ?, ?, ?, ?)", "CREC-003", "CWO-003", "SHIP-003", "Delivered 3", "verified")
	db.ExecContext(ctx, "INSERT INTO cycle_receipts (id, cwo_id, shipment_id, delivered_outcome, status) VALUES (?, ?, ?, ?, ?)", "CREC-004", "CWO-004", "SHIP-003", "Delivered 4", "submitted")

	t.Run("returns true when no CRECs exist (vacuously true)", func(t *testing.T) {
		allVerified, err := repo.AllCRECsVerified(ctx, "SHIP-001")
		if err != nil {
			t.Fatalf("AllCRECsVerified failed: %v", err)
		}
		if !allVerified {
			t.Error("expected true (vacuously), got false")
		}
	})

	t.Run("returns true when all CRECs verified", func(t *testing.T) {
		allVerified, err := repo.AllCRECsVerified(ctx, "SHIP-002")
		if err != nil {
			t.Fatalf("AllCRECsVerified failed: %v", err)
		}
		if !allVerified {
	...
	t.Run("returns false when some CRECs not verified", func(t *testing.T) {
		allVerified, err := repo.AllCRECsVerified(ctx, "SHIP-003")
		if err != nil {
			t.Fatalf("AllCRECsVerified failed: %v", err)
		}
		if allVerified {
